...
import numpy,mwa_impedance,logging,pyfits
import os
...
x_offsets = numpy.array([-1.5,-0.5,0.5,1.5,-1.5,-0.5,0.5,1.5,-1.5,-0.5,0.5,1.5,-1.5,-0.5,0.5,1.5],dtype=numpy.float32)*dipole_sep
y_offsets = numpy.array([1.5,1.5,1.5,1.5,0.5,0.5,0.5,0.5,-0.5,-0.5,-0.5,-0.5,-1.5,-1.5,-1.5,-1.5],dtype=numpy.float32)*dipole_sep


class Dipole:
    """Provides a generic dual pol dipole object to support MWA beam models"""

    def __init__(self,type='short',height=0.29,len=0.74,lookup_filename=os.path.join(filesdir,'Jmatrix.fits'),\
        ...
        """
        assert type=='short' or type=='long' or type=='lookup', 'Unknown type %r' % type
        self.type=type
        self.height=height
        self.len=len
        self.gain=gain
...
class ApertureArray:
    ...
    def __init__(self, dipoles=None, xpos=x_offsets, ypos=y_offsets):
        ...
        """
        assert dipoles == None or len(dipoles)==16, "Expecting 16 input dipoles, got %r" % str(len(dipoles))
        if dipoles == None:
            d = Dipole(type='short')
            dipoles = [d]*16
    ...
    def getResponse(self,az,za,freq=155e6,delays=None):
        ...
        """
        assert delays == None or numpy.size(delays)==32, "Expecting 32 delays, got %r" % str(numpy.size(delays))
        if delays is None:
...
#    d = Dipole('lookup')
    ...
    # az=0, za=14 degs
    delays1=numpy.array([[6,6,6,6,4,4,4,4,2,2,2,2,0,0,0,0],[6,6,6,6,4,4,4,4,2,2,2,2,0,0,0,0]],dtype=numpy.float32)
