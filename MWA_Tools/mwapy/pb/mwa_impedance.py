...
import os,numpy,logging,pyfits
...
class TileImpedanceMatrix:
    ...
    def __init__(self):
        ...
        filename='ZMatrix.fits'
        hdus = pyfits.open(filesdir+'/'+filename)
        nfreqs = len(hdus)
...
#            freqind += 1
        logger.info("Loaded MWA tile impedance matrix with "+str(nfreqs)+" freqs")
    ...
    def getImpedanceMatrix(self,freq):
        ...
        if numpy.abs(self.freqs[pos] - freq) > 5e6:
            logger.warning("Nearest tabulated impedance matrix freq is more than 5 MHz away from desired freq of "+str(freq/1e6)+" MHz")
