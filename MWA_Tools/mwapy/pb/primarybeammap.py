...
def make_primarybeammap(datetimestring, delays, frequency, center=False, sunline=True,
                        ...
                        figsize=8, title=None, directory=None,
                        tle=None, duration=300, verbose=False):
    """
    filename=make_primarybeammap(datetimestring, delays, frequency, center=False, sunline=True,
    low=1, high=2000, plothourangle=True, extension='png', figsize=8, title=None, directory=None, tle=None, duration=300, verbose=False)
    ...
    if len(ra_sat)>0:
        ...
        ax1.scatter(numpy.array(ra_sat)/15.0,numpy.array(dec_sat),
                    #c=numpy.arange(len(ra_sat))/(1.0*len(ra_sat)),cmap=pylab.cm.hsv,
                    c=1-rsat,cmap=pylab.cm.Blues,
                    alpha=0.5,edgecolors='none')
        ax1.text(ra_sat[0]/15.0,dec_sat[0],time_sat[0].strftime('%H:%M:%S'),
                 fontsize=8,horizontalalignment='left',color='c')
        ax1.text(ra_sat[-1]/15.0,dec_sat[-1],time_sat[-1].strftime('%H:%M:%S'),
                 fontsize=8,horizontalalignment='left',color='c')
    ...
    if (plothourangle):
        ...
        if len(satellite_label)>0:
            ax1.text(x1-1,70,satellite_label,
                     fontsize=14,horizontalalignment='left',color='c')
...
def satellite_positions(satellite, observer, time0, DT, RA0=0):
    ...
    for dt in DT:
        ...
        if len(ra_sat)>1 and ra_sat[-1] != numpy.nan:
            if (x>180+RA0 and ra_sat[-1]<180+RA0 and ra_sat[-1]>0+RA0 and x-ephem_utils.putrange(ra_sat[-1],360)<180):
            ...
            if (x<180+RA0 and ephem_utils.putrange(ra_sat[-1],360)>180+RA0 and ephem_utils.putrange(ra_sat[-1],360)-x<180):
                ra_sat.append(numpy.nan)
                dec_sat.append(numpy.nan)
                time_sat.append(compute_time)                
                long_sat.append(satellite.sublong*ephem_utils.DEG_IN_RADIAN)
