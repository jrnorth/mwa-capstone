...
class tile_config():
    ...
    def __init__(self, tile=None, tilename=None,
		 receiver=None, slot=None, inputnumber=None, pol=None, length=None, flag=False, electrical=True, gains=[], delays=[]):
        self.tile=tile
...
class instrument_configuration():
    ...
    ##################################################    
    def __init__(self, gpstime=None, duration=None, db=None, debug=False, min_bad_dipoles=2):
        self.inputs={}
        self.db=db        
...
"""
    ...
    ##################################################    
    def make_instr_config(self, quick=False):
        """
	...
	if quick:
	    ...
	    # get receiver commands to capture per-dipole delays
	    # and slot power info
	    try:
		    recv_cmds=schedule.Obsc_Recv_Cmds(keyval=(receiver,
							      self.gpstime,),
						      db=self.db)
		    logger.info('Retrieved Receiver Commands for observation_number=%d, Rx=%d' % (self.gpstime,receiver))
	    except (psycopg2.InternalError, psycopg2.ProgrammingError) , e:
                    logger.warning('Database error=%s' % (e.pgerror))
                    db.rollback()
		    logger.warning('Unable to retrieve Reciver Commands for observation_number=%d, Rx=%d' % (self.gpstime,receiver))
		    recv_cmds=None

	    if recv_cmds is not None:
		    slot_power=recv_cmds.slot_power
            ...
            if not found_slot_power or slot_power is None or slot_power==[]:
            ...
            for rx_slot in xrange(1,self.ninputsperrx/2+1):
                if recv_cmds is not None:
                    try:
                        xdelays=numpy.array(recv_cmds.xdelaysetting[rx_slot-1])
			ydelays=numpy.array(recv_cmds.ydelaysetting[rx_slot-1])
                    except:
                        xdelays=numpy.array([0]*16)
                        ydelays=numpy.array([0]*16)
		else:
                    xdelays=numpy.array([0]*16)
                    ydelays=numpy.array([0]*16)
                try:
...
# so set it to -1.0
                ...
                # flagging
		# this happens if the delays are NULL
		# make sure it is flagged			
		if xdelays.dtype==numpy.object:
                    self.inputs[x_input_number].flag=True
		    self.inputs[x_input_number].delays=numpy.array([0]*16)
                    logger.info('Flagging tile %dX because delays are NULL' % tile_num)
		if ydelays.dtype==numpy.object:
                    self.inputs[y_input_number].flag=True
		    self.inputs[y_input_number].delays=numpy.array([0]*16)
                    logger.info('Flagging tile %dY because delays are NULL' % tile_num)
                if not slot_power[rx_slot-1] and ((not self.inputs[x_input_number].flag) or (not self.inputs[y_input_number].flag)):
                    self.inputs[x_input_number].flag=True
                    self.inputs[y_input_number].flag=True
		    logger.info('Flagging tile %d because slot_power is False' % tile_num)
                if receiver in self.inactive_receivers:
                    logger.info('Flagging tile %d because it belongs to inactive Rx %s' % (tile_num,receiver))
                    self.inputs[x_input_number].flag=True
                    self.inputs[y_input_number].flag=True
		if ((self.inputs[x_input_number].delays==_BAD_DIPOLE_DELAY).sum()>= self.min_bad_dipoles) and not self.inputs[x_input_number].flag:
			logger.info('Flagging tile %d because %d X dipole(s) are bad' % (tile_num,
											 (self.inputs[x_input_number].delays==_BAD_DIPOLE_DELAY).sum()))
			self.inputs[x_input_number].flag=True
			self.inputs[y_input_number].flag=True			
		if ((self.inputs[y_input_number].delays==_BAD_DIPOLE_DELAY).sum()>= self.min_bad_dipoles) and not self.inputs[x_input_number].flag:
			logger.info('Flagging tile %d because %d Y dipole(s) are bad' % (tile_num,
											 (self.inputs[y_input_number].delays==_BAD_DIPOLE_DELAY).sum()))
			self.inputs[x_input_number].flag=True
			self.inputs[y_input_number].flag=True			
...
#            height = [t.tile_altitude for t in self.tiles if t.tile_id == self.inputs[inputnumber].tile].pop()
	...
	if isinstance(self.channel_selection,list):
        ...
        if (self.obs._MWA_Setting.ra_phase_center is not None):
            head.set('RAPHASE',self.obs._MWA_Setting.ra_phase_center,
                        '[deg] RA of desired phase center')
            head.set('DECPHASE',self.obs._MWA_Setting.dec_phase_center,
        ...
        if self.obs._MWA_Setting is not None:
            head.set('CREATOR',self.obs._MWA_Setting.creator.strip(),'Observation creator')
            head.set('PROJECT',self.obs._MWA_Setting.projectid.strip(),'Project ID')
            head.set('MODE',self.obs._MWA_Setting.mode,'Observation mode')
        if (len(self.obs.receivers)>0):
            head.set('RECVRS',','.join([str(x) for x in self.obs.receivers]),'Active receivers')
	...
	try:
        ...
        head.set('FINECHAN',self.corr2uvfitsheader.fine_channel,'[kHz] Fine channel width')
        head.set('INTTIME',self.corr2uvfitsheader.inttime,'[s] Individual integration time')

        head.set('NSCANS',self.corr2uvfitsheader.n_scans,'Number of scans (time instants) in correlation products')
        head.set('NINPUTS',self.corr2uvfitsheader.n_inputs,'Number of inputs into the correlation products')
        head.set('NCHANS',self.corr2uvfitsheader.n_chans,'Number of fine channels in spectrum')
        head.set('BANDWDTH',self.corr2uvfitsheader.bandwidth,'[MHz] Total bandwidth')
        head.set("FREQCENT",channel2frequency(self.corr2uvfitsheader.channel)+(nav_freq-1)*0.005,
                    '[MHz] Center frequency of observation')
        head.set('TIMEOFF',self.corr2uvfitsheader.timeoffset,
                    '[s] Offset between observation starttime and start of correlations')
        head.set('DATESTRT',self.corr2uvfitsheader.mwatime.strftime('%Y-%m-%d')+'T'+
	...
	Delays=[]
        ...
        for inputnumber in sorted(self.inputs.keys()):
            ...
            Gains.append(numpy.array(self.inputs[inputnumber].gains)[numpy.array(self.obs.channels)][self.channel_selection])
	    Delays.append(numpy.array(self.inputs[inputnumber].delays))
