...
This module defines classes that map onto the main MWA schedule tables:   

mwa_setting
    the MWA_Setting class

rf_stream
    the RFstream class

tile_selection_table
    the Tileset class
...
...
s.rfstreams[0].tileset = schedule.Tileset('all_on', db=db)
s.save(db=db)
...
def getdb(user=None, password=None, host=None, database=None, port=None):
  ...
  try:
    if user is None:
      user = mwaconfig.mandc.dbuser
    if password is None:
      try:
        password = base64.b64decode(mwaconfig.mandc.dbpass)
      except TypeError:
        print "ERROR - bad encoded password in mwa.conf"
    if host is None:
      host = mwaconfig.mandc.dbhost
    if database is None:
      database = mwaconfig.mandc.dbname
    if port is None:
      try:
        port = mwaconfig.mandc.dbport
      except AttributeError:
        port = 5432
  except AttributeError:
    print "ERROR - no database connection parameters passed to schedule.getdb() and params can't be found in mwa.conf"
...
class MWA_Setting(dbobj.dbObject):
  """This class defines a record in the mwa_setting table. A setting is a complete set
     ...
     -starttime, stoptime:
       The times, in GPS seconds, over which this setting is
     ...
     -unpowered_tile_name:
       The name of the set of tiles to power down during this observation

     -modtime:
       a timestamp for when this record was last modified in the
     ...
     -ra_phase_center, dec_phase_center:
       RA and Dec of the commanded phase center,
       in decimal degrees; None if the commanded phase center was not specified in (RA,Dec).
 
     -dataquality integer: (1, 'Good'), (2, 'Some Issues'), (3, 'Unusable'), (4, 'Deleted'), (5, 'Marked for Delete')

     -dataqualitycomment, a text string
     ...
     -rfstreams:
       This can contain a dict of rfstream objects (as many as desired) that form 
     ...
     -voltagebeams:
       This can contain a list of voltage beam objects (these are as yet undefined)
     ...
     -rtsettings:
       This can contain an 'RTSsetting' object (this is as yet undefined) defining
       RTS configuration associated with this setting. If present, it is saved when
       save() is called on the setting object. 
     -logs:
       This can contain a list of MWA_Log objects referring to the observation.
  """
  _table='mwa_setting'
  ...
  int_time=%(int_time)s s, freq_res=%(freq_res)s kHz
  """
  _nmap = {}
  ...
  def __init__(self, keyval=(), db=None):
    ...
    if not self.new:
      self.logs = getLogs(starttime=self.starttime, stoptime=self.stoptime, obsid=self.starttime, db=db)
...
class RFstream(dbobj.dbObject):
  """An object of this class describes a record in the 'rfstream' table. It consists of
     ...
     -number:
       an integer, starting from 0 for the first RF stream associated with a setting

     These two fields (starttime and number) form the primary key of the rf_stream table

     -azimuth, elevation:
       in degrees, None if the beamformer settings are not defined this way

     -ra,dec:
       in degrees, None if the beamformer settings are not defined this way

     -hex:
       Michael's hex beamformer delay specifier, '' if the beamformer settings are not defined this way
     ...
     -tile_selection:
       The name of an entry in the tile_selection_table (text)

     -tileset:
       This can contain an object of the 'Tileset' class. The 'name' of
     ...
     -dipole_exclusion:
       Text field which is the name of the entry in the dipole_exclusion_table
       to use for the tiles in this rf_stream object.

     -frequencies:
       A list (as in the python list type) of up to 24 integer channel numbers, 
       each between 90 and 255, in units of ~1.3 MHz

     -frequency_type:
       an entry from the Frequency_Types table

     -vsib_frequency:
       A value from the frequencies[] list

     -walsh_mode:
       'ON' or 'OFF', or possibly other values in the future.

     -gain_control_type:
       A value from the Gain_Control_Types table

     -gain_control_value:
       Zero, one, or more value(s) in db, 
...
class Tileset(dbobj.dbObject):
  """
  Interface for db/tile_selection_table

  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#tile_selection_table
  ...
  def intersection(self,t2):
    """Performs a set intersection of this tileset with the argument
    with X and Y treated seperately.
  ...
  def remove(self,t2):
    """Removes all of the tiles in the argument from the current tileset
    with X and Y treated seperately.
  ...
  def overlaps(self,t2):
    """
    Determines whether there is any overlap between the current tileset and the argument
...
def makeTileset(db=None, name=None, tilespec=''):
  """Return a tileset object given a tile_selection_table entry name, or a tile specifier, or both.
     ...
     :param db: database object
     :param name: name of existing tileset
     :param tilespec: tile specifier string
     :return: new or existing Tileset object (on success), or None on error
...
class MWA_Log(dbobj.dbObject):
  """
  Interface for db/mwa_log

  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#mwa_log
  ...
  _attribs = [('comment','comment',''),
              ('referencetime','referencetime',None),
              ('logtype','logtype',1),
              ('creator','creator',''),
              ('modtime','modtime',''),
...
class MWA_Log_Types(dbobj.dbObject):
  """
  Interface for db/log_types

  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#log_types
...
class MWA_Obs_Modes(dbobj.dbObject):
  """
  Interface for db/modes

  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#modes
...
class MWA_Gain_Control_Types(dbobj.dbObject):
  """ 
  Interface for db/gain_control_types

  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#gain_control_types
  ...
  def getKey(cls, name=None, db=None):
      ...
      :param name: name to look up
      :param db: database object
      :return: key of desired gain control type
      """
  ...
  def getGainControlType(cls, name=None, db=None):
      ...
      :param name: name to look up
      :param db: database object
      :return: MWA_Gain_Control_Types object
      """
      Gain_Control_Types=cls.getdict(db=db)
      key=None
      for i in Gain_Control_Types.iterkeys():
...
class MWA_Frequency_Types(dbobj.dbObject):
  """
  Interface for db/frequency_types
  
  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#frequency_types
  ...
  def getKey(cls, name=None, db=None):
      ...
      :param name: name to look up
      :param db: database object
      :return: key of desired frequency type
      """
  ...
  def getFrequencyType(cls, name=None, db=None):
      ...
      :param name: name to look up
      :param db: database object
      :return: MWA_Frequency_Types object
      """
      Frequency_Types=cls.getdict(db=db)
      key=None
      for i in Frequency_Types.iterkeys():
...
class MWA_Grid_Points(dbobj.dbObject):
  ...
  Interface for db/grid_points

  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#grid_points
...
class MWA_Data_Files(dbobj.dbObject):
  ...
  Interface for db/data_files

  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#data_files
...
class Obsc_Recv_Cmds(dbobj.dbObject):
  ...
  Interface for db/obsc_recv_cmds
  
  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#obsc_recv_cmds
  ...
  x_gain_control_value
    x_gain_control_value

  y_gain_control_type
    y_gain_control_type  

  y_gain_control_value
    y_gain_control_value
...
class MWA_Project(dbobj.dbObject):
  ...
  Interface for db/mwa_project

  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#mwa_project
...
class Schedule_Metadata(dbobj.dbObject):
  ...
  Interface for db/schedule_metadata
...
class MWA_Tile(dbobj.dbObject):
  ...
  Interface for db/tile_info

  http://mwa-lfd.haystack.mit.edu/twiki/bin/view/Main/DatabaseImplementation#tile_info
...
def checkhex(hexcode, nwords=16, separator=","):
    """  
    checks whether the supplied hexcode is valid
    the hexcode should be <nwords> * 6-bit words, separated by <separator>

    :param hexcode: hexcode to check (string)
    :param nwords: number of words that the hexcode should be (int)
    :param separator: separator between the words (string)
...
def getSettings(db=None, pagesize=20, page=None, mintime=None, maxtime=None):
  """Return a list of MWA_Setting objects in database. If maxtime or
     ...
     :param page: number of page to return or GPStime of an entry on that page
     :param mintime: minimum time to return
     :param maxtime: maximum time to return
     :return: list of MWA_Setting objects
...
def getStartstoptimes(db=None, mintime=None, maxtime=None, key='stoptime', sort=False):
    ...
    :param db: database object
    :param mintime: minimum time to search
    :param maxtime: maximum time to search
    :param key: key to use for searching
    :param sort: should the results be sorted?
    :return: starttime,stoptimes for matching entries, or None
    """
    if mintime is None:
        mintime = 0
    if maxtime is None:
...
def getLogs(starttime=0, stoptime=2147483647,
            obsid=None,
            order='order by referencetime desc',
            logtypes=None,
            limit=1000000,
            db=None):
  """
  Return the logs that match a given query.  Can limit search to starttime and stoptime, and also select by obsid and logtype

  :param starttime: starttime for query
  :param stoptime: stoptime for query
  :param obsid: obsid for query
  :param order: SQL order command for query
  :param logtypes: logtypes for query
  :param limit: maximum number of results to return
  :param db: database object
  :return: list of MWA_Log entries that match
  """

  if logtypes is None:
    typeclause = " 't' "
  else:
    typeclause = " logtype = ANY (%(logtypes)s) "
  logids = dbobj.execute('select log_id from mwa_log ' +
                              'where ( ' + typeclause + ' ) and ' +
                                    '( ( (referencetime < %(stoptime)s) and '
                                        '(endtime > %(starttime)s) )   or ' +
                                      '( (referencetime >= %(starttime)s) and '
                                        '(referencetime < %(stoptime)s) and '
                                        '(endtime is NULL) ) ) ' +
                              order + ' ' +
                              'limit %(limit)s',
                         {'starttime':starttime, 'stoptime':stoptime, 'obsid':obsid,
                              'limit':limit, 'logtypes':logtypes},
                         db=db)
  logids += dbobj.execute('select log_id from mwa_log ' +
                              'where ( ' + typeclause + ' ) and ' +
                                    '( ( (observation >= %(starttime)s) and '
                                        '(observation < %(stoptime)s) )    or ' +
                                      '( (%(obsid)s is not NULL) and (observation = %(obsid)s) ) ) ' +
                              order + ' ' +
                              'limit %(limit)s',
                         {'starttime':starttime, 'stoptime':stoptime, 'obsid':obsid,
                              'limit':limit, 'logtypes':logtypes},
  ...
  for s in logids:
    ...
    if 'referencetime' in order.lower():
      logs.sort(cmp=lambda x,y: cmp((x.referencetime or x.observation),(y.referencetime or y.observation)))
    if 'endtime' in order.lower():
      logs.sort(cmp=lambda x,y: cmp((x.endtime or x.referencetime),(y.endtime or y.referencetime)))
    if 'modtime' in order.lower():
      logs.sort(cmp=lambda x,y: cmp(x.modtime,y.modtime))
    if 'observation' in order.lower():
      logs.sort(cmp=lambda x,y: cmp((x.observation or x.referencetime),(y.observation or y.referencetime)))
...
def getdata_andpickle(databasecommand, picklename, timedifference=86400, db=None):
  ...
  :param databasecommand: reference to python function such as MWA_Log_Types.getdict or MWA_Grid_Points.getall
  :param picklename: name to pickle for results
  :param timedifference: how often should new results be retrieved (in s)
  :param db: database object
  :return: database entries that were retrieved
...
def getDefaultGainType(db=None):
      ...
      :param db: database object
      :return: default Gain_Control_Type index or None
...
def getDefaultObsMode(db=None):
      ...
      :param db: database object
      :return: default MWA_Obs_Mode index or None
      """
      try:
        modes=Obs_Modes
      except:
...
def getDefaultFrequencyType(db=None):
      ...
      :param db: database object
      :return: default MWA_Frequency_Types index or None
      """
      try:
