    ...
    def __setattr__(self, name, value):
        ...
        if (name == 'observation_number' and value is not None):
            ...
            try:
                ...
                logger.warning('Database error=%s' % (e.pgerror))
                db.rollback()
            # no longer get the frequencies here.  Instead get them
            # from the receiver commands
            #try:
                    ...
                    for rx in xrange(len(self.receivers)):
                        ...
                        if (recv_cmds.stoptime > 0):
                            try:
                                ...
                                # with psycopg, no longer need to go from string to array
                                self.delays=recv_cmds.xdelaysetting[0]
                                logger.info('Found delays in Obsc_Recv_Cmds for Rx %d (%s)\n' % (self.receivers[rx],','.join([str(x) for x in self.delays])))
                                self.channels=recv_cmds.frequency_values
                                self.center_channel=self.channels[len(self.channels)/2]
                                break
                            except:
