...
then
     ...
     hostmem="10gb"
# Fornax node storage is currently inoperable (2014-02-18)
#     ngascommands="/scratch/$groupq/MWA/ngas/ngamsCClient -host 202.8.39.136 -port 7777 -mimeType application/octet-stream -cmd PARCHIVE -nexturl http://192.168.222.$3:7777/QARCHIVE -fileUri "
     ngascommands="/scratch/$groupq/MWA/ngas/ngamsCClient -host store04.icrar.org -port 7777 -cmd QARCHIVE -mimeType application/octet-stream -fileUri "
 else
     computer="fornax"
     groupq="partner676"
     standardq="workq"
     hostmem="30gb"
     ngasclient="/scratch/mwaops/ngas/ngamsCClient"
# Fornax node storage is currently inoperable (2014-02-18)
#     ngascommands="/scratch/mwaops/ngas/ngamsCClient -host 192.168.222.$3 -port 7777 -cmd QARCHIVE -mimeType  application/octet-stream -fileUri "
     ngascommands="/scratch/mwaops/ngas/ngamsCClient -host store04.icrar.org -port 7777 -cmd QARCHIVE -mimeType application/octet-stream -fileUri "
 fi
