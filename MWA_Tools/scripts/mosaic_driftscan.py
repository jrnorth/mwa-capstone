...
import logging, sys, os, glob, string, re, urllib, math, time, shutil
...
try:
    ...
    from mwapy import make_metafiles
    _newmetadata=False
...
class MWAImage():
    ...
    def __init__(self, obsid=None):
        ...
        self.yweight=None
        self.iweight=None
        self.xscalarweight=None
        self.yscalarweight=None
        self.iscalarweight=None
...
######################################################################        
def reduce_dimensionality(image):
    """
    newfile=reduce_dimensionality(image)
    return a 2D version of the image
    """
    ...
    f.writeto(newfile)
    f.close()
    logger.info('Wrote 2D version of %s to %s' % (image, newfile))
    return newfile

######################################################################        
def weightfunction(value, function):
    assert function in ['inversesquare','inverse','linear','square']
    if function=='inversesquare':
        return 1/value**2
...
def main():
    ...
    parser.add_option('--scale','--scalar',dest='scalarweight',default=None,
                      help='Additional scalar weight header keyword for each image [default=None]')
    parser.add_option('--function',dest='weightfunction',default='inversesquare',
                      type='choice',
                      choices=['inversesquare','inverse','linear','square'],
                      help='Function applied to scalar weight (higher weight is better) [default=%default]')
    parser.add_option('--separate',dest='separate',default=False,
                      action='store_true',
    ...
    for obsid in sorted(Imagelist.keys()):
        ...
        if Imagelist[obsid].iimage is None:
            ...
            if options.scalarweight is not None:
                try:
                    Imagelist[obsid].xscalarweight=weightfunction(fx[0].header[options.scalarweight],
                                                                  options.weightfunction)
                except:
                    logger.warning('Unable to read header keyword "%s" from image %s; setting I scalar weight to 1' % (options.scalarweight, Imagelist[obsid].ximage))
                    Imagelist[obsid].xscalarweight=1
                try:
                    Imagelist[obsid].yscalarweight=weightfunction(fy[0].header[options.scalarweight],
                                                                  options.weightfunction)
                except:
                    logger.warning('Unable to read header keyword "%s" from image %s; setting I scalar weight to 1' % (options.scalarweight, Imagelist[obsid].yimage))
            ...
            if not options.separate:
                ...
                Imagelist[obsid].ximage=Imagelist[obsid].ximage.replace('_XX','_XXcorr')
                if os.path.exists(Imagelist[obsid].ximage):
                    os.remove(Imagelist[obsid].ximage)
                fx.writeto(Imagelist[obsid].ximage)
                logger.info('Wrote corrected XX image %s' % Imagelist[obsid].ximage)
                tempfiles.append(Imagelist[obsid].ximage)
                # and copy the XX beam to correspond to the new name
                shutil.copy(Imagelist[obsid].xbeam,
                            Imagelist[obsid].xbeam.replace('_XX','_XXcorr').replace('_YY','_YYcorr'))
                logger.info('Copied XX beam from %s -> %s' % (Imagelist[obsid].xbeam,
                                                              Imagelist[obsid].xbeam.replace('_XX','_XXcorr').replace('_YY','_YYcorr')))                
                Imagelist[obsid].xbeam=Imagelist[obsid].xbeam.replace('_XX','_XXcorr').replace('_YY','_YYcorr')
                tempfiles.append(Imagelist[obsid].xbeam)
                fy[0].data=(fy[0].data/fyB[0].data)
                Imagelist[obsid].yimage=Imagelist[obsid].yimage.replace('_YY','_YYcorr')
                if os.path.exists(Imagelist[obsid].yimage):
                    os.remove(Imagelist[obsid].yimage)
                fy.writeto(Imagelist[obsid].yimage)
                logger.info('Wrote corrected YY image %s' % Imagelist[obsid].yimage)
                tempfiles.append(Imagelist[obsid].yimage)
                # and copy the YY beam to correspond to the new name
                shutil.copy(Imagelist[obsid].ybeam,
                            Imagelist[obsid].ybeam.replace('_XX','_XXcorr').replace('_YY','_YYcorr'))
                logger.info('Copied YY beam from %s -> %s' % (Imagelist[obsid].ybeam,
                                                              Imagelist[obsid].ybeam.replace('_XX','_XXcorr').replace('_YY','_YYcorr')))                
                Imagelist[obsid].ybeam=Imagelist[obsid].ybeam.replace('_XX','_XXcorr').replace('_YY','_YYcorr')
    ...
    for obsid in sorted(Imagelist.keys()):
        if not options.separate:
            ...
            if options.reduce:
                newfile=reduce_dimensionality(Imagelist[obsid].iimage)
                newroot,newext=os.path.splitext(newfile)
                Imagelist[obsid].iimage=newfile
                tempfiles.append(newfile)
            else:
                newroot, newext=os.path.splitext(Imagelist[obsid].iimage)
            ...
            if options.scalarweight is not None:
                ...
                try:
                    Imagelist[obsid].iscalarweight=weightfunction(f[0].header[options.scalarweight],
                                                                  options.weightfunction)
                except:
                    logger.warning('Unable to read header keyword "%s" from image %s; setting I scalar weight to 1' % (options.scalarweight, Imagelist[obsid].iimage))
                    Imagelist[obsid].iscalarweight=1
               
            f=pyfits.open(Imagelist[obsid].ibeam)               
            f[0].data[f[0].data<options.threshold]=0
            f[0].data=f[0].data[0,0]**2
            # apply the scalar weight if needed
            if Imagelist[obsid].iscalarweight is not None:
                logger.info('Weighting beam for %s by %f' % (Imagelist[obsid].iimage, Imagelist[obsid].iscalarweight))
                f[0].data*=Imagelist[obsid].iscalarweight
            if f[0].header['CRVAL1']<0:
                f[0].header['CRVAL1']+=360
            ...
            Imagelist[obsid].iweight=weightfile
            tempfiles.append(weightfile)

        else:
            # do this for X and Y images separately
            root,ext=os.path.splitext(Imagelist[obsid].ximage)    
            if options.reduce:
                newfile=reduce_dimensionality(Imagelist[obsid].ximage)
                newroot,newext=os.path.splitext(newfile)
                Imagelist[obsid].ximage=newfile
                tempfiles.append(newfile)
            else:
                newroot, newext=os.path.splitext(Imagelist[obsid].ximage)

            weightfile=newroot + newsuffix
            # Get scaling from imageheader
            if options.scalarweight is not None:
                f=pyfits.open(Imagelist[obsid].ximage)
                try:
                    Imagelist[obsid].xscalarweight=weightfunction(f[0].header[options.scalarweight],
                                                                  options.weightfunction)
                except:
                    logger.warning('Unable to read header keyword "%s" from image %s; setting I scalar weight to 1' % (options.scalarweight, Imagelist[obsid].ximage))
                    Imagelist[obsid].xscalarweight=1
               
            f=pyfits.open(Imagelist[obsid].xbeam)               
            f[0].data[f[0].data<options.threshold]=0
            f[0].data=f[0].data[0,0]**2
            # apply the scalar weight if needed
            if Imagelist[obsid].xscalarweight is not None:
                logger.info('Weighting beam for %s by %f' % (Imagelist[obsid].ximage, Imagelist[obsid].xscalarweight))
                f[0].data*=Imagelist[obsid].xscalarweight
            if f[0].header['CRVAL1']<0:
                f[0].header['CRVAL1']+=360
            f[0].header['BMTHRESH']=(options.threshold,'Threshold for primary beam correction')
            f.verify('fix')
            ...
            f.close()
            logger.info('Wrote weight file to %s' % weightfile)
            Imagelist[obsid].xweight=weightfile
            tempfiles.append(weightfile)

            root,ext=os.path.splitext(Imagelist[obsid].yimage)    
            if options.reduce:
                newfile=reduce_dimensionality(Imagelist[obsid].yimage)
                newroot,newext=os.path.splitext(newfile)
                Imagelist[obsid].yimage=newfile
                tempfiles.append(newfile)
            else:
                newroot, newext=os.path.splitext(Imagelist[obsid].yimage)

            weightfile=newroot + newsuffix
            # Get scaling from imageheader
            if options.scalarweight is not None:
                f=pyfits.open(Imagelist[obsid].yimage)
                try:
                    Imagelist[obsid].yscalarweight=weightfunction(f[0].header[options.scalarweight],
                                                                  options.weightfunction)
                except:
                    logger.warning('Unable to read header keyword "%s" from image %s; setting I scalar weight to 1' % (options.scalarweight, Imagelist[obsid].yimage))
                    Imagelist[obsid].yscalarweight=1
               
            f=pyfits.open(Imagelist[obsid].ybeam)               
            f[0].data[f[0].data<options.threshold]=0
            f[0].data=f[0].data[0,0]**2
            # apply the scalar weight if needed
            if Imagelist[obsid].yscalarweight is not None:
                logger.info('Weighting beam for %s by %f' % (Imagelist[obsid].yimage, Imagelist[obsid].yscalarweight))
                f[0].data*=Imagelist[obsid].yscalarweight
            if f[0].header['CRVAL1']<0:
                f[0].header['CRVAL1']+=360
            f[0].header['BMTHRESH']=(options.threshold,'Threshold for primary beam correction')
            f.verify('fix')
            ...
            f.close()
            logger.info('Wrote weight file to %s' % weightfile)
            Imagelist[obsid].yweight=weightfile
            tempfiles.append(weightfile)
    ...
    for stokes in Stokes:
        logger.info('Processing polarization %s...' % stokes)
        ...
        for obsid in sorted(Imagelist.keys()):
            if stokes=='I':
                ...
                images.append(Imagelist[obsid].ximage)
            elif stokes=='Y':
                images.append(Imagelist[obsid].yimage)

            if center is None:
                # only determine the center once
                # so all results are consistent
                if stokes=='I':
                    f=pyfits.open(Imagelist[obsid].iimage)
        ...
        if len(options.swarpcommands)>0:
            command+=' ' + options.swarpcommands
        if stokes=='I':
            outname='%s.swarp.fits' % options.root
            outweightname='%s.weight.fits' % options.root
        elif stokes=='X':
            outname='%s.swarpX.fits' % options.root
            outweightname='%s.weightX.fits' % options.root
        elif stokes=='Y':
            outname='%s.swarpY.fits' % options.root
            outweightname='%s.weightY.fits' % options.root
