...
def main():
    ...
    parser.add_option('--intfreq',dest='intfreq',default=1,type=int,
                      help='Specify frequency averaging factor; raw data is 40kHz (32T) or 10kHz (128T) so e.g. to average from 10kHz to 40kHz, use --intfreq 4 [default: %default]')
    ...
    extragroup.add_option('--autoflag',dest='autoflag',default=True,action='store_true',
                          help='Use autoflagging in corr2uvfits? [default: %default]')
    extragroup.add_option('--noautoflag',dest='autoflag',default=True,action='store_false',
                          help='Do not use autoflagging in corr2uvfits? [default: %default]')
    ...
    extragroup.add_option('-l','--lock',dest='lock',action='store_true',default=False,
    ...
    try:
        ...
        if observation_num is None and files[0] is not None:
    ...
    correct_chan=splat_average.splat_average.channel_order(observation.center_channel)
    ...
    if (not os.path.exists(outname_ac) or force):
        ...
        retval=splat_average.splat_average.splat_average_ac(ac_names, outname_ac, ntimes, 
                                              _NDAS*_CHANSPERDAS, 
                                              _NINP, 
                                              inttime, intfreq, correct_chan)
        if retval is None:
    ...
    if (not os.path.exists(outname_cc) or force):
        ...
        retval=splat_average.splat_average.splat_average_cc(cc_names, outname_cc, ntimes, 
                                              _NDAS*_CHANSPERDAS, 
                                              _NINP, 
                                              inttime, intfreq, correct_chan)
        if retval is None:
    ...
    corr2uvfits=Corr2UVFITS(basename=options.root,RA=ra,Dec=dec,
                            ...
                            conjugate=conjugate,correlator=correlator,timeoffset=timeoffset,lock=options.lock,force=force,
                            fake=False)
...
##################################################
    ...
    def __init__(self,basename=None,objectname=None,RA=None,Dec=None,
                 ...
                 inttime=1,intfreq=1,totaltime=None,flag=0,flagfile=None,
                 antenna_locations=None,instr_config=None,
                 conjugate=0,correlator='s',timeoffset=2,
                 lock=False,force=False,fake=None):
        """
