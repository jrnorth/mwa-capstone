...
spw=vis+'/SPECTRAL_WINDOW'
ants=vis+'/ANTENNA'
field=vis+'/FIELD'

vis_root=re.sub('.ms','',vis)

# read the file with the auto numpy filereader
amps=n.loadtxt('gains.txt',skiprows=1)
phases=n.loadtxt('phases.txt',skiprows=1)

newamps=n.delete(amps,0,1)
newphases=n.delete(phases,0,1)

# Reshape
nramps=n.reshape(newamps,(768,4,128))
nrphases=n.reshape(newphases,(768,4,128))

# Transpose to match casaformat
nnamps=np.transpose(nramps,(1,0,2))
nnphases=np.transpose(nrphases,(1,0,2))

# Then combine to make complex G
G=(nnamps*n.cos(nnphases)+complex(0,1)*nnamps*n.sin(nnphases))
...
# Fake mask and flags
M = n.empty(shape=(shape(G)), dtype=bool)
...
#--------------------Plot options--------------------------
phase_png=re.sub('.ms','_phases.png',vis)
snr_png=re.sub('.ms','_snr.png',vis)
amp_png=re.sub('.ms','_amp.png',vis)
...
phase_title="{0:s}: {1:s} @ {2:3.0f}--{3:3.0f}MHz: {4:s} UTC ({5:s} WST)".format(vis_root,calibrator,(minfreq/1e6),(maxfreq/1e6),obs_time_wst,obs_time_utc)

amp_title="{0:s}: {1:s} @ {2:3.0f}--{3:3.0f}MHz: {4:s} UTC ({5:s} WST): Scale = 0--{6:2.1f}".format(vis_root,calibrator,(minfreq/1e6),(maxfreq/1e6),obs_time_wst,obs_time_utc,maxamp)

# XX raw data and fit in green; YY raw data and fit in red
# XY and YX some more random colors
#datacolor=('g','r','b','y')
datacolor=('g','b','y','r')
fitcolor=('c','g','r','m')
legend_position=(0.50,0.95)
# XX, YY labels. assuming XX=0 and YY=3
pol_label=('XX','XY','YX','YY')

#--------------------------------------------------------------

for antenna in range(G.shape[2]):
# Don't bother with XY, YX
